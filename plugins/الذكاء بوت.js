...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `[🤖]⌯ اضــف سـؤال الــي الـذكــاء الـاصـطـناعـي\n\n⌯ مــثــال :\n⌯ ${usedPrefix + command} تــحــدث عـن حـيـاه الــرســول\n⌯ ${usedPrefix + command} افـضـل مــواقــع تــعــلـم الـبـرمـجـه بـالـمـجان`;
  try {
        conn.sendPresenceUpdate('composing', m.chat);
        let sistema1 = `نانو بوت`;
        ...
        if (respuesta == 'error' || respuesta == '' || !respuesta) return XD; // 
        m.reply(`${respuesta}`.trim());
    } catch {
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const botIA222 = await openaiii.createCompletion({model: 'text-davinci-003', prompt: text, temperature: 0.3, max_tokens: 4097, stop: ['Ai:', 'Human:'], top_p: 1, frequency_penalty: 0.2, presence_penalty: 0});
        if (botIA222.data.choices[0].text == 'error' || botIA222.data.choices[0].text == '' || !botIA222.data.choices[0].text) return XD; // 
        m.reply(botIA222.data.choices[0].text.trim());
    } catch {
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const syms1 = `  اوبيتو بوت`;
      ...
      try {
        ...
        throw `⌯ حــدث خــطــأ غـيـر مــتــوقــع`;
     }}
    }}
   }}
  }}
