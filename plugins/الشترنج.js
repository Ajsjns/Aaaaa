...
const handler = async (m, { conn, args }) => {
  ...
  if (args[0] && args[1]) {
    ...
    const turn = `『OBITO-𝘽𝙊𝙏』 *دور:* ${currentcolor} @${chessdata.currentTurn.split('@')[0]}\n\n${chess.getComment() || ''}`;
    const flipParam = senderid === gamedata.black ? '' : '&flip=true';
    const flipParam2 = senderid === gamedata.black ? '' : '-flip';
    const boardUrl = `https://www.chess.com/dynboard?fen=${encodedfen}&board=graffiti&piece=graffiti&size=3&coordinates=inside${flipparam}`;
    try {
...
بعدها اكتب *شطرنج ادخل* عشان تخش انت وصاحبك اللعبه.
  ...
  return conn.reply(m.chat, '『OBITO-𝘽𝙊𝙏』“ امر خاطئ. استعمل*"شطرنج مساعده"* عشان تشوف الاوامر شغاله ازاي.', m);
};
